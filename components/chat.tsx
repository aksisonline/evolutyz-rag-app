...
export function Chat({
  ...
  useEffect(() => {
    // Load from localStorage without session dependency
    const savedFiles = localStorage.getItem('selected-file-pathnames');
    if (savedFiles) {
      setSelectedFilePathnames(JSON.parse(savedFiles));
    }
  }, []);

  useEffect(() => {
    // Save to localStorage without session dependency
